...
internal sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BRZE Hero Effect Ability Config Probe V8 — AUTO / READ ONLY";
        ...
        Controls.Add(new Label{Text="HERO EFFECT ABILITY CONFIG PROBE V8 — ISSYL A5 vs GRAYBACK C0",Font=new Font("Segoe UI Semibold",18f),AutoSize=true,Location=new Point(24,18)});
        Controls.Add(new Label{Text="READ ONLY • auto-detect A5/C0 • parent+0x1F4 config record • focus config+0x0E0 • automatic expiry",AutoSize=true,ForeColor=Color.FromArgb(125,205,170),Location=new Point(27,57)});
        ...
        var arm=B("ARM TARGET + AUTO DETECT",24,260);
        var reset=B("RESET",296,120);
        var copy=B("COPY REPORT",428,160);
        ...
        Controls.Add(new Label{Text="TEST A: select ONE clean target -> ARM -> cast ORIGINAL Issyl Haste once -> wait COMPLETE -> COPY REPORT. RESET. TEST B: same target style -> ARM -> cast ORIGINAL Grayback effect once -> wait natural expiry -> COPY REPORT. No manual timing needed.",AutoSize=false,Location=new Point(24,238),Size=new Size(1388,72),ForeColor=Color.FromArgb(180,195,218)});
