...
internal sealed class HeroEffectIntegratedPanel : IntegratedFeatureCard
{
    ...
    readonly System.Windows.Forms.Timer timer=new(){Interval=80};
    ...
    public HeroEffectIntegratedPanel()
    {
        ...
        actions.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,134));
        ...
        seconds.Dock=DockStyle.Fill;seconds.Margin=new Padding(6,3,0,3);actions.Controls.Add(seconds,4,0);
        root.Controls.Add(actions,0,1);

        phase.Text="Select one or more units, set duration in seconds, then APPLY ISSYL / APPLY GRAYBACK / APPLY BOTH. No baseline capture needed.";
        root.Controls.Add(phase,0,2);
        Controls.Add(root);

        issyl.Click+=(_,_)=>Apply(DirectHeroMode.Issyl);
        gray.Click+=(_,_)=>Apply(DirectHeroMode.Grayback);
        both.Click+=(_,_)=>Apply(DirectHeroMode.Both);
        timer.Tick+=(_,_)=>RefreshHero();timer.Start();
    }

    void Apply(DirectHeroMode mode)
    {
        phase.Text=HeroEffectDirectCore.Start(mode,seconds.Value);
        SetEnabled(!HeroEffectDirectCore.Busy());
    }

    void RefreshHero()
    {
        phase.Text=HeroEffectDirectCore.Tick();
        SetEnabled(!HeroEffectDirectCore.Busy());
    }

    void SetEnabled(bool enabled)
    {
        issyl.Enabled=enabled;gray.Enabled=enabled;both.Enabled=enabled;seconds.Enabled=enabled;
    }

    protected override void Dispose(bool disposing)
    {
        if(disposing){timer.Stop();HeroEffectDirectCore.Shutdown();timer.Dispose();}
