...
internal sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BRZE Hero Effect Transient Probe V5 — AUTO / READ ONLY";
        ...
        Controls.Add(new Label{Text="HERO EFFECT TRANSIENT PROBE V5 — AUTO TIMER HUNT",Font=new Font("Segoe UI Semibold",18f),AutoSize=true,Location=new Point(24,18)});
        Controls.Add(new Label{Text="READ ONLY • automatic effect start/end • 50 ms sampling • changing fields ranked before static lifecycle data",AutoSize=true,ForeColor=Color.FromArgb(125,205,170),Location=new Point(27,57)});
        ...
        {
            Text="FLOW: select ONE clean normal unit that will receive the buff -> ARM TARGET + AUTO WATCH -> now you may change selection if needed -> cast ORIGINAL ISSYL (recommended) or Grayback ONCE on that target -> do nothing else. V5 auto-detects effect start, samples transient objects every 50 ms, and auto-finishes after the proven lifecycle roots return to baseline for 3 samples. No fast human clicking required.",
