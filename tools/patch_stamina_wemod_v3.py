...
Path('StaminaCore.cs').write_text(r'''using System;
...
# Add StaminaCore to the publish project.
p=Path('MergedTrainer.csproj')
s=p.read_text(encoding='utf-8')
needle='    <Compile Include="HorseCore.cs" />\n'
if needle not in s: raise SystemExit('missing HorseCore compile item')
s=s.replace(needle,needle+'    <Compile Include="StaminaCore.cs" />\n',1)
p.write_text(s,encoding='utf-8')

# Rewire F5 away from the failed Add/Set implementation and into StaminaCore.
p=Path('tools/prepare_merged_v1.py')
s=p.read_text(encoding='utf-8')
s=s.replace('F5 Unlimited Stamina (selected only) — HARD LOCK','F5 Unlimited Stamina (selected only) — WAND READ-SITE',1)
old='Native.Start(); timer.Tick += (_, _) => TickTrainer(); timer.Start(); FormClosed += (_, _) => { HorseCore.Stop(); HookCore.Stop(); SelectionCore.Stop(); Native.Stop(); };'
new='Native.Start(); timer.Tick += (_, _) => TickTrainer(); timer.Start(); FormClosed += (_, _) => { StaminaCore.Stop(); HorseCore.Stop(); HookCore.Stop(); SelectionCore.Stop(); Native.Stop(); };'
if old not in s: raise SystemExit('missing close wiring')
s=s.replace(old,new,1)
old="    'string hookStatus = HookCore.Tick(f5.Checked, f6.Checked, f7.Checked);')"
new="    'string hookStatus = HookCore.Tick(false, f6.Checked, f7.Checked);\\n        string staminaStatus = StaminaCore.Tick(f5.Checked);')"
if old not in s: raise SystemExit('missing HookCore wiring')
s=s.replace(old,new,1)
old=' + "\\\\r\\\\n" + hookStatus + "\\\\r\\\\n" + horseStatus + "\\\\r\\\\n" + selectionStatus;\')'
new=' + "\\\\r\\\\n" + hookStatus + "\\\\r\\\\n" + staminaStatus + "\\\\r\\\\n" + horseStatus + "\\\\r\\\\n" + selectionStatus;\')'
if old not in s: raise SystemExit('missing status wiring')
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')
print('StaminaCore + wiring generated')
