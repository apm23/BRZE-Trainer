...
if old3 not in src:
    raise SystemExit('V36 v4: reveal retry marker missing')
src=src.replace(old3,new3,1)

# Never embed/copy the PNG payload inside this wrapper. Use the exact checked-in
# 64x64 base64 asset, which also makes payload length/validity auditable in CI.
...
if n!=1:
    raise SystemExit('V36 v4: icon payload marker missing')

exec(compile(src,'finalize_v36_stress_hardening.v4.generated.py','exec'),{'__name__':'__main__'})
