...
internal static class ReplayCore
{
    ...
    static bool InstallHook()
    {
        ...
        long site=moduleBase+RVA_FRAME_MOUSE_DRAW; var now=new byte[FrameOriginal.Length]; if(!ReadExact(site,now)||!Same(now,FrameOriginal)){status="FRAME HOOK BUSY/MISMATCH — close main trainer / Clone Lab / Sniffer";return false;}
        ...
        installed=true;status="REPLAY V2 GAME-THREAD HOOK ACTIVE";return true;
    }
    ...
    public static string QueueSelected(uint ability1,uint ability2,string label)
    {
        lock(sync)
        {
            var units=SelectedUnits(); if(units.Count==0)return $"{label}: no units selected."; if(!InstallHook())return $"{label}: blocked — {status}"; long c=cave.ToInt64();
            ...
            lastQueue=$"{label}: queued {units.Count} selected";return lastQueue;
        }
    }
