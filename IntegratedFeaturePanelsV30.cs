...
internal sealed class HeroEffectIntegratedPanel : IntegratedFeatureCard
{
    ...
    public HeroEffectIntegratedPanel()
    {
        ...
        root.Controls.Add(SectionTitle("HERO EFFECT  //  APPLY / RESET"),0,0);
        ...
        phase.Text="Select unit(s) and APPLY. If the same buff is already active, its existing instance restarts from now — no stacking.";
        ...
        timer.Tick+=(_,_)=>phase.Text=HeroEffectDirectCoreV30.Tick();timer.Start();
    }

    void Apply(DirectHeroMode mode)=>phase.Text=HeroEffectDirectCoreV30.Start(mode,seconds.Value);

    protected override void Dispose(bool disposing)
    {
        if(disposing){timer.Stop();HeroEffectDirectCoreV30.Shutdown();timer.Dispose();}
